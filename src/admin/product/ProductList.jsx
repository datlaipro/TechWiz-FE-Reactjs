...
function ProductList() {
  ...
  const truncate = (txt, n = 80) => (!txt ? '' : txt.length <= n ? txt : txt.slice(0, n) + '…');
  ...
  useEffect(() => {
    const filtered = events.filter((ev) => {
      const titleMatch = (ev.title || '').toLowerCase().includes(searchTerm.trim().toLowerCase());
      ...
      const startRaw = ev.start_date || ev.startDate || ev.date || ev.Date || '';
  ...
  const handleFilterChange = (name) => (e) => setFilters((p) => ({ ...p, [name]: e.target.value }));
  const clearFilters = () => setFilters({ department: '', category: '', dateFrom: '', dateTo: '' });
  ...
  const handleView = (id) => navigate(`/admin/eventview/${id}`);
  const handleEdit = (id) => navigate(`/admin/editevent/${id}`);
  ...
  const inputSx = {
    '& .MuiOutlinedInput-root': {
      ...
      '&.Mui-focused fieldset': { borderColor: 'primary.main', boxShadow: '0 0 8px rgba(25,118,210,0.2)' },
    },
  };
  ...
  return (
    <Box sx={{ mt: 2, px: { xs: 2, sm: 4 }, maxWidth: '1400px', mx: 'auto' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexWrap: 'wrap', gap: 2 }}>
        <Typography variant="h5" sx={{ fontWeight: 'bold', color: '#1a2820', letterSpacing: '0.5px' }}>
          DANH SÁCH SỰ KIỆN
        </Typography>
        <Button
          variant="contained"
          color="primary"
          onClick={handleAdd}
          sx={{ borderRadius: '20px', textTransform: 'none', fontWeight: 'medium', px: 3, py: 1,
            boxShadow: '0 4px 12px rgba(0,0,0,0.1)',
            '&:hover': { boxShadow: '0 6px 16px rgba(0,0,0,0.15)', bgcolor: 'primary.dark' } }}
        >
          Thêm sự kiện
        </Button>
      </Box>
      ...
      {/* Bộ lọc */}
      <Paper sx={{ mb: 2, p: 2, borderRadius: '12px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
          ...
          <FormControl sx={{ minWidth: 180 }} size="small">
            ...
            <Select label="Phòng ban" value={filters.department} onChange={handleFilterChange('department')} sx={inputSx}>
          ...
          <FormControl sx={{ minWidth: 180 }} size="small">
            ...
            <Select label="Loại sự kiện" value={filters.category} onChange={handleFilterChange('category')} sx={inputSx}>
              <MenuItem value="">Tất cả</MenuItem>
              {categoryOptions.map((c) => <MenuItem key={c} value={c}>{c}</MenuItem>)}
            </Select>
          </FormControl>
          <TextField size="small" label="Từ ngày" type="date" value={filters.dateFrom}
                     onChange={handleFilterChange('dateFrom')} InputLabelProps={{ shrink: true }} sx={{ minWidth: 180, ...inputSx }} />
          <TextField size="small" label="Đến ngày" type="date" value={filters.dateTo}
                     onChange={handleFilterChange('dateTo')} InputLabelProps={{ shrink: true }} sx={{ minWidth: 180, ...inputSx }} />
          <Button variant="outlined" onClick={clearFilters} sx={{ ml: 'auto' }}>XÓA LỌC</Button>
        </Box>
      </Paper>

      {/* Ô tìm kiếm */}
      <TextField
        fullWidth label="Tìm kiếm theo tiêu đề" value={searchTerm} onChange={handleSearchChange}
        margin="normal" variant="outlined"
        sx={{
          mb: 3,
          '& .MuiOutlinedInput-root': { borderRadius: '12px', backgroundColor: 'background.paper',
            '&:hover fieldset': { borderColor: 'primary.main' },
            '&.Mui-focused fieldset': { borderColor: 'primary.main', boxShadow: '0 0 8px rgba(25,118,210,0.3)' } },
        ...
        <Table sx={{ minWidth: 650 }} aria-label="event table">
          <TableHead>
            <TableRow sx={{ backgroundColor: 'grey.100',
              '& th': { fontWeight: 'bold', color: 'text.primary', py: 2, borderBottom: '2px solid', borderColor: 'grey.300' } }}>
          ...
          <TableBody>
            {filteredEvents.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((ev) => {
              ...
              return (
                <TableRow key={id}
                  sx={{ '&:hover': { backgroundColor: 'grey.50', transition: 'background-color 0.2s' },
                        '& td': { py: 1.5, borderBottom: '1px solid', borderColor: 'grey.200' } }}>
                  <TableCell>{id}</TableCell>
                  <TableCell sx={{ fontWeight: 'medium' }}>{title}</TableCell>
                  <TableCell>{category}</TableCell>
                  <TableCell sx={{ maxWidth: 260 }}>
                    <Typography variant="body2" color="text.secondary">{truncate(desc, 90)}</Typography>
                  </TableCell>
                  ...
                  <TableCell>
                    <IconButton onClick={() => handleView(id)} sx={{ color: 'info.main', '&:hover': { bgcolor: 'info.light', transform: 'scale(1.1)' }, transition: 'all 0.2s' }}>
                      <VisibilityIcon />
                    </IconButton>
                    <IconButton onClick={() => handleEdit(id)} sx={{ color: 'warning.main', '&:hover': { bgcolor: 'warning.light', transform: 'scale(1.1)' }, transition: 'all 0.2s' }}>
                      <EditIcon />
                    </IconButton>
                    <IconButton onClick={() => handleDelete(id)} sx={{ color: 'error.main', '&:hover': { bgcolor: 'error.light', transform: 'scale(1.1)' }, transition: 'all 0.2s' }}>
                      <DeleteIcon />
                    </IconButton>
                  </TableCell>
                </TableRow>
      ...
      <TablePagination
        ...
        sx={{
          mt: 2,
          '& .MuiTablePagination-toolbar': { backgroundColor: 'grey.50', borderRadius: '8px', py: 1 },
          '& .MuiTablePagination-selectLabel, & .MuiTablePagination-displayedRows': { color: 'text.secondary', fontWeight: 'medium' },
          '& .MuiTablePagination-actions button': { borderRadius: '8px', '&:hover': { bgcolor: 'grey.200' } },
        }}
      />
    </Box>
  );
